...
async def process_hypothesis(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Save to database
        hypothesis_data = {
            "title": result.get("processed_hypothesis", initial_state["input"])[:255],
            "description": initial_state["input"],
            "thesis": result.get("processed_hypothesis", ""),
            ...
            for contradiction in result["contradictions"]:
                if isinstance(contradiction, dict):
                    contradiction_data = {
                        ...
                        "quote": contradiction.get("content", ""),
                        "reason": f"Contradiction identified: {contradiction.get('type', '')}",
